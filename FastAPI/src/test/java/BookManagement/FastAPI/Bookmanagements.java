...
public class Bookmanagements {
	...
	public void BookManagement() {
		...
		LoginResponse loginres = given().spec(req).body(login).when().post(FastAPIResources.Login()).then().spec(res200)
				.extract().response().as(LoginResponse.class);

		accessToken = loginres.getAccess_token();
		System.out.println("User logged in successfully");

		// -----------------------CreateBook for the FASTAPI
		// -Bookmanagement----------------

		CreateBookRequest createBook = new CreateBookRequest();
		createBook.setAuthor("John Will");
		createBook.setBook_summary(
				"Dive into the world of REST APIs and master the art of automated testing with this practical, hands-on guide.");
		...
		CreateBookResponse newBook = given().spec(reqAuth).body(createBook).when().post(FastAPIResources.CreateBook())
				.then().spec(res200).extract().response().as(CreateBookResponse.class);

		String bookID = newBook.getId();

		System.out.println("The Book has been created with the id: " + bookID);

		// -----------------------Get books by Book_ID for the FASTAPI
		// -Bookmanagement----------------

		GetBookByIDResponse getbookByID = given().pathParam("book_id", bookID).spec(reqAuth).when()
				.get(FastAPIResources.GetBookById()).then().spec(res200).extract().response()
		...
		// -----------------------UPDATE books using Book_ID for the FASTAPI
		// -Bookmanagement----------------

		UpdateBookRequest updateBook = new UpdateBookRequest();
		updateBook.setAuthor("Rahul Shetty");
		updateBook.setBook_summary(
				"This comprehensive resource takes you on a journey from the foundational principles of RESTful architecture and HTTP methods to the intricacies of building robust test automation frameworks.");
		...
		UpdateBookResponse updatebook = given().pathParam("book_id", bookID).spec(reqAuth).body(updateBook).when()
				.put(FastAPIResources.UpdateBookByID()).then().spec(res200).extract().response()
		...
		// -----------------------Delete books by using Book_IDfor the FASTAPI
		// -Bookmanagement----------------

		DeleteBookByIDResponse deleteBookID = given().pathParam("book_id", bookID).spec(reqAuth).body(updateBook).when()
				.delete(FastAPIResources.DeleteBookByID()).then().spec(res200).extract().response()
